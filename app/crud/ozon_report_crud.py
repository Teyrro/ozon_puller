...
class CRUDOzonReport(CRUDBase[OzonReport, IOzonReportCreate, IOzonReportUpdate]):
    async def get_last_item_by_created_at(self, *, db_session: AsyncSession | None = None) -> OzonReport | None:
        stmt = (select(OzonReport).where(OzonReport.report_type == ReportType.seller_products.lower())
                .order_by(desc(OzonReport.ozon_created_at)).limit(1))
        session = db_session or self.db.session
        ozon_info = await session.execute(stmt)
        return ozon_info.scalar_one_or_none()

    async def create_seller_report_for_all_users(self, *, users: list[User],
