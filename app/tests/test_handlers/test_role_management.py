...
async def test_add_admin_role_to_user_by_superadmin(
    ...
    resp = client.patch(
        ...
        headers=create_test_auth_for_user(user_data_who_promoted["email"]),
    )
...
async def test_revoke_admin_role_from_user_by_superadmin(
    ...
    resp = client.delete(
        ...
        headers=create_test_auth_for_user(user_data_who_revoke["email"]),
    )
    data_from_resp = resp.json()
    assert resp.status_code == 200
...
async def test_revoke_admin_role_from_user_by_wrong_type_of_user(
    ...
    assert data_from_resp == {"detail": "This user don't have enough privileges"}
