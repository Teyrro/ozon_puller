...
from app.db.models import PortalRole
...
async def test_delete_user_not_found(client, create_user_in_database):
    ...
    user_data_for_database["user_id"] = await create_user_in_database(
        user_data_for_database
    )
    user_data["user_id"] = await create_user_in_database(user_data)
...
async def test_delete_user_user_id_validation_error(client, create_user_in_database):
    ...
    user_data["user_id"] = await create_user_in_database(user_data)
    resp = client.delete(
        "/user/?user_id=123", headers=create_test_auth_for_user(user_data["email"])
    )
...
async def test_user_not_auth(client, create_user_in_database):
    ...
    resp = client.delete(
        f"/user/?user_id={user_data['user_id']}", headers=bad_auth_headers
    )
...
async def test_delete_user_by_privilege_roles(
    ...
    user_data_for_deletion["user_id"] = await create_user_in_database(
        user_data_for_deletion
    )
    user_data["user_id"] = await create_user_in_database(user_data)
    resp = client.delete(
    ...
    user_from_db = await get_user_from_database(user_data_for_deletion["user_id"])
...
async def test_delete_another_user_error(
    ...
    user_for_deletion["user_id"] = await create_user_in_database(user_for_deletion)
    user_who_delete["user_id"] = await create_user_in_database(user_who_delete)
    resp = client.delete(
        f"/user/?user_id={user_for_deletion['user_id']}",
        headers=create_test_auth_for_user(user_who_delete["email"]),
    )
...
async def test_reject_delete_superadmin(
    ...
    resp = client.delete(
        f"/user/?user_id={user_for_deletion['user_id']}",
        headers=create_test_auth_for_user(user_for_deletion["email"]),
    )
    ...
    assert PortalRole.ROLE_PORTAL_SUPERADMIN in user_from_database.roles
